...
2) I created a obsidian account and intend to sign-up for a student discount (later decided against this as I am using a github.)
...
4) When starting the Arch install process for the first time
	...
	4) This above time management methodology is not ideal, as I later discovered that once I made it past the step of configuring and formatting the partitions, devoting time in large increments like this isn't necessary. 
...
12) BELIEVE TO BE A PARTITION ERROR, ERASING THE VM and start over, I have saved a quick reference of commands to speed things up for the next round.  REFERENCE ERRORS IN  STEP 9
...
14) after some refresher , I figured out that the standard password is '**root**' and I have set the password. saved in a separate markdown file in obsidian
...
STARTED WORKING ON PROJECT AGAIN, CHANGED FONT TO ' **using setfont /usr/share/kbd/consolefonts/iso02-12x22.psfu.gz**' for east of readability, as it's a larger font then default.
18) added user "josh" to the "wheel" group, using '**usermod -aG wheel josh**' user password set using '**passwd josh**'and saved in separate obsidian file, using 
19) Created user 'Codi' using ' **useradd -m Codi**', added Codi to the "wheel" group using '**usermod -aG wheel Codi**', set Codi password '**password Codi**' using  user password set and saved in separate obsidian file. 
